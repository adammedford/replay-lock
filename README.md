...
`review` displays each pending candidate's callable, arguments, return or throw completion, eligibility evidence, provenance, and occurrence count. Accept, reject, or skip each candidate explicitly. Acceptance writes a deterministic case; later behavior is an explicit old/new replacement decision, never an automatic expected-value update. A fourth answer, `af` (accept remaining in this file), still prints every subsequent same-module candidate's full review output before accepting it — a keystroke-saving convenience over the same per-candidate atomic write, never a way to accept a candidate without seeing it.
