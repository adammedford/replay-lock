...
test('32 fixed seeds in Node and Chromium preserve completions, order, mutation and native counts; replay is offline',{timeout:120000},async()=>{
  ...
  for(const realm of ['node','browser']){const r=result.realms[realm];assert.equal(r.executed,96);assert.equal(r.replayed,84);assert.equal(r.excluded,12);assert.equal(r.throws,6);assert.deepEqual(r.faults,{observerFailures:true,capacity:1001,detachedRejected:true,intrinsicsGuarded:true});}
});
