...
`npm run yield:dev -- --root <project>` reports how many callables development capture can instrument and, for the rest, the share of skipped callables carrying each reason code. Add `--blockers` to see why: each skipped callable's blockers, meaning a reason code at its root site (the end of the diagnostic's cause chain, grouped by package for dependencies). Callables whose shape is outside what capture supports are counted apart from those blocked in their own body or only through inherited sites. It then lists the sites that block the most callables, a greedy plan of which sites to resolve to make the most callables eligible, and callables with at most two blockers. Unlock counts are upper bounds, because analysis records one origin per code per callable. `--markdown <file>` writes a snapshot, such as [the Epic Stack report](epic-blockers-2026-09.md); `--json` includes every callable's blockers. `test/acceptance/dev-yield.test.mjs` enforces the fixture expectations described in [the yield fixtures](../../test/fixtures/yield/README.md). Each analyzer change that affects eligibility records its before/after counts here.
